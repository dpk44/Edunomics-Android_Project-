...
class ValidatePhotos {
    ...
    private fun checkIfValidImage(
        postingDraftPhoto: PhotoFile,
        canAddThisToList: Boolean
    ...
    private fun complyAspectRatio(photoWidth: Int, photoHeight: Int, ruleValue: Double): Boolean {
        ...
        if (photoWidth != 0 && photoHeight != 0) {
            val ratio =
                if (photoWidth > photoHeight) photoWidth.toDouble() / photoHeight else photoHeight.toDouble() / photoWidth
    ...
    private fun validatePhoto(postingDraftPhoto: PhotoFile) =
        !postingDraftPhoto.path.isNullOrBlank()

    fun checkIfMimeTypeSupported(mimeType: String): Boolean {
        return mimeType.isEmpty() || listOf("image/png", "image/jpeg", "image/webp").contains(
            mimeType
        )
    }
}
